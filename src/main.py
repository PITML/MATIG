...
def main(rank, world_size, cli_args, extras):
    ...
    if rank == 0:
        ...
        dump_config(os.path.join(config.exp_dir, config.trial_name, 'MATIG_noLVIS.yaml'), config)
    ...
    logit_scale = DDP(logit_scale, device_ids=[rank], output_device=rank, find_unused_parameters=True)
    image_proj = DDP(image_proj, device_ids=[rank], output_device=rank, find_unused_parameters=True)
    text_proj = DDP(text_proj, device_ids=[rank], output_device=rank, find_unused_parameters=True)
    ...
    if config.resume is not None:
        ...
        print("autoresume")
        print("loade test", os.path.join(config.ckpt_dir, '{}.pt'.format('latest')))
        if os.path.exists(os.path.join(config.ckpt_dir, '{}.pt'.format('latest'))):
            print("loade before", os.path.join(config.ckpt_dir, '{}.pt'.format('latest')))
    ...
    if config.train:
        ...
        logging.info("\n\nevalute LVIS datasets:")
        trainer.load_from_checkpoint(config.evalute_lvis_model_path)
        trainer.test_objaverse_lvis()

        logging.info("\n\nevalute ModelNet40 datasets:")
        trainer.load_from_checkpoint(config.evalute_modelnet40_model_path)
